...
type Photo struct {
	...
	Url             string      `json:"url"`
	Photographer    string      `json:"photograher"`
...
func (c *Client) CurratedPhotos(perPage, page int) (*CuratedResult, error) {
...
func (c *Client) dataRandomPhoto() (*Photo, error) {
	rand.Seed(time.Now().Unix())
	randNum := rand.Intn(1001)
	result, err := c.CurratedPhotos(1, randNum)
...
func main() {
	os.Setenv("PexelsToken", "1NBgAiU6habhv9LdVs6yeUxph92RvT6l7uBGUHPk4AT0E9uKK1t2CMka")
	var TOKEN = os.Getenv("PexelsToken")
	var c = NewClient(TOKEN)

	result, err := c.SearchPhotos("waves", 10, 1)
	if err != nil {
